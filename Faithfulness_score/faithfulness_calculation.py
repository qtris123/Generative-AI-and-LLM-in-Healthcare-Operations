...
class FaithfulnessScore:
    def __init__(self, mode="SHAP"):
        if mode == 'LIME':
            ...
            explainer = Explainer('SHAP') # explainer in this case stands for SHAP
            self.not_trained = explainer.not_trained #during the first attempt (when not_trained == True), a RF model and its SHAP explanation for X_train data should alr be saved. Hence, no need to run the SHAP explainer again.
            self.shap_explainer = explainer.explainer_model # SHAP explainer  
            self.model_shap = explainer.model
            explainer_2 = Explainer('LIME')
            self.lime_explainer = explainer_2.explainer_model #LIME explainer
            self.model_lime = explainer_2.model
    ...
    def _top_k_features_of_row(self, row_ind=21516, k =5, num_cycles = 50):
    ...
    def _compute_faithfulness_dataset(self, metric="topk", k = 5, num_cycles = 1000, num_rows = 1): 
        ...
        Returns:
            faithfulness:
        """
        number_of_disturbance = num_rows
        top_k_features = {}
        ans = []
        ...
        for i in range(1,num_cycles + 1):
            ...
            all_N_map = torch.tensor([False] * len(feature_contribution_scores), dtype=torch.bool)
            baseline_score = k*self._compute_faithfulness_topk(x, c_label, all_N_map)
            faith_score = ((faithfulness / num_rows) / baseline_score)*100  # ?? add a division by num_cycles.
            ...
            faith_min = 3.945
            faith_max = 49.393 ## these values are from shap_1000_3
            """""
            ...
            """
            if faith_score < faith_min:
                min_distr.append(list(indices))
            if faith_score > faith_max:
                max_distr.append(list(indices))
        

        return ans, top_k_features, min_distr, max_distr
    
    # calculate the faithfulness score considering an n sample of the test data, and take the average. => the representative faith score for the data
    def _compute_faithfulness_a_row(self, metric="topk", k = 5, num_rows = 50): 
        """Computes AUC for faithfulness scores, perturbing top k (where k is an array).
        ...
        for row_ind in tqdm(indices):
            ...
            if self.mode == "LIME":
                ...
                for k_i in range(1,k+1):
                    ...
                    for _ in range(10):
                        if metric == "topk":
                            row_faithfulness += self._compute_faithfulness_topk(x, c_label, top_k_map) 
                        else:
                            # Otherwise, provide bottom-k indices
                            row_faithfulness += self._compute_faithfulness_topk(x, c_label, ~top_k_map)

                    faithfulness += row_faithfulness/10
            ...
            """""
                ...
                #LIME
                # select explanation values for one instance/row; at the moment, take row indexed 9 as an example.
                exp_instance_lime = self.lime_explainer.explain_instance(np.array(self.test_data[self.test_data.index == row_ind])[0], self.model_lime.predict_proba, num_features=14, top_labels=k)
                c_label_lime = list(exp_instance_lime.predict_proba).index(max(exp_instance_lime.predict_proba)) #the most likely class
                feature_contribution_scores_lime = exp_instance_lime.as_map()[c_label_lime]
                for k_i in range(1,k+1):
                    # Construct original mask as all true (i.e., all indices are masked and non are perturbed)
                    top_k_map_lime = torch.tensor([True] * len(feature_contribution_scores_lime), dtype=torch.bool)

                    # Unmask topk instances 
                    top_k_indices_lime = [x[0] for x in exp_instance_lime.as_map()[c_label_lime]][:k_i]
                    top_k_map_lime[top_k_indices_lime] = False
                    #self.top_k_features += Counter({self.feature_names[i]: 1 for i, flag in enumerate(top_k_map) if not flag.item()}

                    # If top-k provide top-k instances 
                    x_lime = np.array(self.test_data[self.test_data.index == row_ind]) # x input for compute_faithfulness_topk is an np.array with reshape(1,-1) or without [0]
                    
                    row_faithfulness_lime = 0
                    for _ in range(10):
                        if metric == "topk":
                            row_faithfulness_lime += self._compute_faithfulness_topk_lime(x_lime, c_label_lime, top_k_map_lime) 
                        else:
                            # Otherwise, provide bottom-k indices
                            row_faithfulness_lime += self._compute_faithfulness_topk_lime(x_lime, c_label_lime, ~top_k_map_lime)

                    faithfulness_lime += row_faithfulness_lime/10
                ...
                sum_features = np.sum(explanation[ind,:,:].values, axis = 0) # dim(n_features x n_classes)
                base_values = self.shap_explainer.expected_value # dim(1 x n_classes) #use shap_explainer not explanation
                c_label_shap = np.argmax(base_values + sum_features) # the most likely class for an instance
                feature_contribution_scores_shap = explanation[ind,:,c_label_shap]

                for k_i in range(1,k+1):
                    # Construct original mask as all true (i.e., all indices are masked and non are perturbed)
                    top_k_map_shap = torch.tensor([True] * len(feature_contribution_scores_shap), dtype=torch.bool)
                    # Unmask topk instances 
                    top_k_indices_shap = list(np.argsort(abs(feature_contribution_scores_shap.values))[-k_i:])
                    top_k_map_shap[top_k_indices_shap] = False

                    # If top-k provide top-k instances 
                    x_shap = np.array(self.test_data[self.test_data.index == row_ind]) # x input for compute_faithfulness_topk is an np.array with reshape(1,-1) or without [0]
                    
                    row_faithfulness_shap = 0
                    for _ in range(10):
                        if metric == "topk":
                            row_faithfulness_shap += self._compute_faithfulness_topk_shap(x_shap, c_label_shap, top_k_map_shap) 
                        else:
                            # Otherwise, provide bottom-k indices
                            row_faithfulness_shap += self._compute_faithfulness_topk_shap(x_shap, c_label_shap, ~top_k_map_shap)

                    faithfulness_shap += row_faithfulness_shap/10


            #LIME
            for rank, indx in enumerate(top_k_indices_lime, start=1):
                feature_name  = self.feature_names[indx]
                if feature_name not in top_k_features_lime:
                    top_k_features_lime[feature_name] = {}

                rank_key = f"rank_{rank}"
                # Increment the count for this rank or set it to 1 if it doesn't exist yet
                top_k_features_lime[feature_name][rank_key] = top_k_features_lime[feature_name].get(rank_key, 0) + 1

            #SHAP
            for rank, indx in enumerate(top_k_indices_shap, start=1):
                feature_name  = self.feature_names[indx]
                if feature_name not in top_k_features_shap:
                    top_k_features_shap[feature_name] = {}

                rank_key = f"rank_{rank}"
                # Increment the count for this rank or set it to 1 if it doesn't exist yet
                top_k_features_shap[feature_name][rank_key] = top_k_features_shap[feature_name].get(rank_key, 0) + 1
        

            #calculate the baseline:
            all_N_map = torch.tensor([False] * len(feature_contribution_scores_shap), dtype=torch.bool) # either ...shape or ...lime is fine.
            if (self.mode == 'All'):
                baseline_score_lime = k*self._compute_faithfulness_topk_lime(x_lime, c_label_lime, all_N_map) 
                baseline_score_shap = k*self._compute_faithfulness_topk_shap(x_shap, c_label_shap, all_N_map) 
                faith_score_lime = ((faithfulness_lime) / baseline_score_lime)*100 
                faith_score_shap = ((faithfulness_shap) / baseline_score_shap)*100 
                ans.append((faith_score_lime, faith_score_shap))
                faith_score_lime = 0
                faith_score_shap = 0
            else:
        """   
            ...
            all_N_map = torch.tensor([False] * len(feature_contribution_scores), dtype=torch.bool) # either ...shape or ...lime is fine.
            baseline_score = k*self._compute_faithfulness_topk(x, c_label, all_N_map) 
            faith_score = ((faithfulness) / baseline_score)*100 
            ans.append(faith_score)
            total_faithfulness += faith_score
        
        if (self.mode == 'All'):
            return ans, indices, top_k_features_lime, top_k_features_shap
        else:
            return total_faithfulness, ans, indices, top_k_features
    ...
    def _compute_faithfulness_topk_lime(self, x, label, top_k_mask, num_samples: int = 10_000):
        """Approximates the expected local faithfulness of the explanation in a neighborhood.
        ...
        x_perturbed = self.perturbation_method.get_perturbed_inputs(**perturb_args)
        # TODO(satya): Could you make these lines more readable?
        y = self._arr([i[label] for i in self._arr(self.model_lime.predict_proba(x.reshape(1, -1)))])
        y_perturbed = self._arr([i[label] for i in self._arr(self.model_lime.predict_proba(x_perturbed.float()))])

        # Return abs mean
        return np.mean(np.abs(y - y_perturbed), axis=0)
    
    def _compute_faithfulness_topk_shap(self, x, label, top_k_mask, num_samples: int = 10_000):
        """Approximates the expected local faithfulness of the explanation in a neighborhood.
        ...
        x_perturbed = self.perturbation_method.get_perturbed_inputs(**perturb_args)
        # TODO(satya): Could you make these lines more readable?
        y = self._arr([i[label] for i in self._arr(self.model_shap.predict_proba(x.reshape(1, -1)))])
        y_perturbed = self._arr([i[label] for i in self._arr(self.model_shap.predict_proba(x_perturbed.float()))])
...
#print(f'the faithful score for a row is: {obj._compute_faithfulness_a_row()}') # actually a row # only case where we use mode = 'All'
print("----------------------------------------------------------") 
print(f'the faithful score for dataset is: {obj._compute_faithfulness_dataset()}') # actually a dataset
